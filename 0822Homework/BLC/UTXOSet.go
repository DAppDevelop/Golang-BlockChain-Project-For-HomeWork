...
func (utxoset *UTXOSetYS) ResetUTXOSetYS() {
	...
	err := utxoset.blockChainYS.DBYS.Update(func(tx *bolt.Tx) error {
		//1.utxoset表存在，删除
...
func (utxoSet *UTXOSetYS) FindUnspentUTXOsByAddressYS(address string) []*UTXOYS {
	...
	//读数据库
	err := utxoSet.blockChainYS.DBYS.View(func(tx *bolt.Tx) error {
		b := tx.Bucket([]byte(UTXOSetBucketNameYS))
		if b != nil {
		...
		if b != nil {
			...
			for _, input := range txInputs {
				...
				if isNeedDelete {
					...
					if len(utxos) > 0 {
						outputs := &TxOutputsYS{utxos}
						b.Put(input.TxIDYS, outputs.SerializeYS())
					}
				}
			...
			for txID, outputs := range outsMap {
				...
				b.Put(txIDBytes, outputs.SerializeYS())
			}
