...
type WalletYS struct {
	PrivateKeyYS ecdsa.PrivateKey //1.私钥
	PublickKeyYS []byte           //2.公钥  原始公钥
}

/*
	创建新的钱包
 */
func NewWalletYS() *WalletYS {
	privateKey, publicKey := newKeyPairYS()

	return &WalletYS{privateKey, publicKey}
}

//产生一对密钥
func newKeyPairYS() (ecdsa.PrivateKey, []byte) {
	/*
	1.根据椭圆曲线算法，产生随机私钥
	2.根据私钥，产生公钥
	椭圆：ellipse，
...
func (w *WalletYS) GetAddressYS() []byte {
	...
	address := PublicHashToAddressYS(pubKeyHash)
...
2.ripemd160
 */
func PubKeyHashYS(publickKey []byte) []byte {
	//1.sha256
	hasher := sha256.New()
...
两次sha256
 */
func CheckSumYS(payload [] byte) []byte {
	firstHash := sha256.Sum256(payload)
	secondHash := sha256.Sum256(firstHash[:])
	//取前四个字节
	return secondHash[:addressCheckSumLenYS]

}

//校验地址是否有效：
func IsValidAddressYS(address []byte) bool {
	//step1：Base58解码
	//version+pubkeyHash+checksum
	full_payload := Base58Decode(address)

	//step2：获取地址中携带的checkSUm
	checkSumBytes := full_payload[len(full_payload)-addressCheckSumLenYS:]
	version_payload := full_payload[:len(full_payload)-addressCheckSumLenYS]
...
func PublicHashToAddressYS(pubKeyHash []byte) []byte {
