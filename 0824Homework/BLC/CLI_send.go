...
func (cli *CLIYS) sendYS(from []string, to []string, amount []string, nodeID string, mine bool) {
	/*
	...
	if mine {
		fmt.Println("--------------本地挖矿")
		txs := blockchain.hanldeTransationsYS(from, to, amount, nodeID)
		newBlock := blockchain.MineNewBlockYS(txs)
		blockchain.SaveNewBlockToBlockchainYS(newBlock)
		...
		if nodeAddressYS != knowNodesYS[0] {
			...
			sendTransactionToMainNodeYS(knowNodesYS[0], txs)
		} else {
			//如果交易是在主节点, 直接发送给挖矿节点
